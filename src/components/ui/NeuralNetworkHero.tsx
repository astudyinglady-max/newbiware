...
/* Fix: Expanded JSX augmentation to include both global JSX and React.JSX namespaces. 
   This ensures that Three.js elements like 'mesh' and 'planeGeometry' are recognized 
   by the TypeScript compiler regardless of the React JSX transform configuration. */
declare global {
  namespace JSX {
...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    buf[0] = sigmoid(buf[0]);
    // Light theme: Invert or wash out the colors
    vec3 color = vec3(1.0 - buf[0].x * 0.1, 1.0 - buf[0].y * 0.05, 1.0);
...
export default function NeuralNetworkHero({
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-full overflow-hidden flex flex-col justify-center">
    
      <div className="relative mx-auto flex max-w-[1600px] w-full flex-col items-start gap-8 px-6 pb-24 pt-20 md:px-10 lg:px-16">
        ...
        <h1 
          ref={headerRef} 
          className="max-w-6xl text-left text-5xl font-bold leading-[1.1] tracking-tight text-[#ffffff] sm:text-7xl md:text-8xl lg:text-[100px] font-syne"
          dangerouslySetInnerHTML={{ __html: title }}
        />

        <p ref={paraRef} className="max-w-3xl text-left text-lg font-light leading-relaxed tracking-tight text-white text-3xl"
