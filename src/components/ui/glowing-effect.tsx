...
const GlowingEffect = memo(
    ({
        ...
        useEffect(() => {
            ...
            if (active) {
                // Auto-animate loop for active state
                let currentAngle = 0;
                const animateLoop = () => {
                    if (!containerRef.current) return;
                    currentAngle += 1; // Speed of rotation
                    if (currentAngle >= 360) currentAngle = 0;

                    containerRef.current.style.setProperty("--active", "1");
                    containerRef.current.style.setProperty("--start", String(currentAngle));
